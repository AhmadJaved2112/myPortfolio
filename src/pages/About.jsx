  ...
  return (
    <div id='about'>
      <div>
        ...
        <p className='text-gray-700 dark:text-[#A8A29E] mt-4 md:mt-6 w-full md:w-3/4 lg:w-1/2 text-wrap text-base sm:text-lg md:text-xl leading-relaxed'>
      ...
      <div className='flex gap-5 md:gap-10 my-16'>
        {
          Experience.map((item) => (
            <div key={item.id} className='text-text'>
              <h3 className='dark:text-white text-4xl md:text-5xl lg:text-7xl font-extrabold'>{item.numbers}</h3>
              <p className='text-text-gray-700 dark:text-[#A8A29E] uppercase text-lg sm-text-sm break-word'>{item.label1} <br /> {item.label2}</p>
            </div>
          ))
        }
      </div>
      ...
      {/*  */}
      <button className='cursor-pointer mt-4 text-white bg-linear-to-r from-[#b0c216] to-[#F37016] w-28 h-10 rounded-md  hover:scale-105 duration-300 transition-all'>
        <a
